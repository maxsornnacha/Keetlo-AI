...
export default defineEventHandler(async (event) => {
  ...
  const html = await getPageId(id, config)
...
async function getPageId(pageId: string, config: { public: { NUXT_PUBLIC_API_BASE: string } }) {
    try{
  const apiBase = config.public.NUXT_PUBLIC_API_BASE;
  const data = await $fetch<{ htmlContent: string }>(`${apiBase}/public/projects/page/${pageId}`)
  return data?.htmlContent ?? '<div/>'
