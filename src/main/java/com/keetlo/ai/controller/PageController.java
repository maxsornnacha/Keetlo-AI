...
public class PageController {
  ...
  public PageController(ObjectMapper objectMapper, PageService pageService, MessageService messageService,
      WebClient openAiClient, ImageTokenReplacer imageTokenReplacer, FileUtil fileUtil) {
  ...
  private String buildSystemPrompt() {
    String systemPrompt = """
        ...
        - Every reveal element **must include**: `class="reveal opacity-0 translate-y-4 duration-500 ease-out"`. You may add other classes, but these **must** be present.
  ...
  public Flux<String> generatePages(@RequestBody CreateMessageStreamReq request) {
    try {
      ...
      String userInput = request.getInput();
        ...
        .put("role", "system")
        .put("content", this.buildSystemPrompt()));
        ...
        .put("type", "text")
        .put("text", "User intent: " + userInput));
        }

        // Expecting frontend to send: List<FileDto> files (fileName, fileType, fileSize, base64)
  ...
  public ResponseEntity<?> uploadPage(@RequestBody Page request) {
    ...
    try {
      String generatedPageId = pageService.createPage(projectMessageId, label, path, htmlContent, height, width, top,
          left);
  ...
  public void updatePage(@Payload Page request) {
    try {
      pageService.updatePageByGeneratedPageId(request.getGeneratedPageId(), request.getHeight(), request.getWidth(),
          request.getTop(), request.getLeft());
    } catch (Exception e) {
      System.out.println(e.getMessage());
    }
  }
    ...
    try {
      ...
      if (deleted) {
        ...
        return ResponseEntity.status(404)
            .body(Collections.singletonMap("error", "Page not found or not owned by user"));
      }
    } catch (Exception e) {
