...
<template>
  <main class="relative min-h-screen text-white pb-8 lg:pb-16">
    ...
    <div
      class="flex pt-10 justify-center pointer-events-none absolute inset-0 -z-10 overflow-hidden"
    >
      <div
        class="z-[1] w-full h-full max-w-[500px] max-h-[500px] rounded-full blur-xl opacity-70 bg-no-repeat bg-[radial-gradient(120%_90%_at_25%_25%,_var(--tw-gradient-stops))] [--tw-gradient-stops:#60a5fa,_#22d3ee_30%,_#a78bfa_55%,_#f97316_75%,_#facc15_100%]"
      />
    </div>
    ...
    <section class="mx-auto w-full max-w-5xl px-4 py-12 md:py-16 text-center">
      <h2
        class="text-5xl md:text-7xl font-black leading-tight tracking-[-0.033em] text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-200"
      >
    ...
    <div class="mx-auto max-w-[1400px] px-4 lg:px-8">
      <section
        class="w-full p-4 lg:p-6 border border-slate-50/20 rounded-xl bg-[#0D1117]"
      >
        <div
        ...
        >
          ...
          <div class="relative w-full md:max-w-md">
            <UIcon
              ...
              class="absolute left-3 top-1/2 -translate-y-1/2 size-5 text-white/70"
            />
            <input
              v-model="query"
              type="search"
              placeholder="Search projects, users, tags…"
              class="w-full rounded-xl bg-white/10 border border-white/10 pl-10 pr-3 py-2.5 placeholder:text-white/50 text-white focus:outline-none focus:ring-2 focus:ring-indigo-400/50"
            />
            <button
              v-if="query"
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <UIcon
                :name="
                  dense ? 'i-lucide-layout-grid' : 'i-lucide-layout-dashboard'
                "
        ...
        <div class="mt-4 flex items-center gap-2">
          <div class="relative flex-1 overflow-x-auto scrollbar-none">
            <div
              class="pointer-events-none absolute left-0 top-0 h-full w-6 bg-gradient-to-r from-[#0D1117] to-transparent"
            />
            <div
              class="pointer-events-none absolute right-0 top-0 h-full w-6 bg-gradient-to-l from-[#0D1117] to-transparent"
            />

            <!-- Skeleton pills while loading tags -->
            <div
              v-if="loadingTags"
              class="flex items-center gap-2 pr-6 pl-6 w-max"
            >
    ...
    <!-- Public Projects -->
    <div class="mx-auto max-w-[1400px] px-4 lg:px-8 mt-4 lg:mt-8">
      <section
        class="w-full p-4 lg:p-6 border border-slate-50/20 bg-[#0D1117] rounded-xl"
      >
        ...
        <div v-if="!loadingProjects && totalCount === 0">
          <div
            class="mx-auto text-center rounded-2xl border border-white/10 bg-white/5 p-8"
          >
            <div
              class="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-full bg-white/10"
            >
              <UIcon name="i-lucide-search" class="size-8 text-white/80" />
            </div>

            <h3 class="text-xl font-bold text-white">No results found</h3>
            <p class="mt-2 text-white/70">
              We couldn’t find anything that matches your filters. Try adjusting
            ...
            >
              <button
                class="cursor-pointer w-full sm:w-auto rounded-lg bg-white/10 px-4 py-2 text-sm text-white hover:bg-white/15 border border-white/10"
                @click="page = 1"
              >
                Go to first page
              </button>
              ...
              <button
                class="cursor-pointer w-full sm:w-auto rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700"
                @click="
                  typeof resetAll === 'function'
                    ? resetAll()
                    : ((query = ''), (selectedTags = []))
                "
              >
            ...
            <div
              class="mt-6 flex flex-wrap items-center justify-center gap-2 text-sm"
            >
              <span class="text-white/60">Try:</span>
              <button
                class="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-white/80 hover:bg-white/15"
                @click="
                  query = 'landing page';
                  page = 1;
                "
        ...
        >
          ...
          <template v-if="loadingProjects">
            ...
            >
              ...
              >
                <div
                  class="h-full w-full animate-pulse bg-gradient-to-b from-white/10 to-white/5"
                />
              </div>
          ...
          <template v-else>
              ...
              >
                ...
                <div
                  ...
                  class="group relative w-full rounded-2xl overflow-hidden border border-slate-50/10 h-[220px]"
                >
                  <div
                    class="absolute inset-0 z-[2] transition duration-300 bg-white/10 group-hover:lg:bg-black/70 pointer-events-none"
                  />
                  ...
                  <!-- floating actions -->
                  <div
                    class="hidden lg:block absolute inset-0 gap-2 z-[20] pointer-events-none"
                  >
                    <div
                  ...
                  <div
                    class="lg:hidden absolute inset-0 gap-2 z-[20] pointer-events-none"
                  >
                    <div
                      class="flex justify-end items-start p-4 gap-2 h-full transition duration-300 pointer-events-auto"
                    >
                      <button
                        class="rounded-lg p-3 text-white/90 cursor-pointer bg-black/40 hover:bg-black/50"
                        @click="openPreview(project)"
                      >
                        Preview
                      </button>
                      ...
                      <button
                        v-if="loadingFavorite"
                        class="rounded-lg pt-2 pb-1 px-2 text-white/90 cursor-default bg-black/40 hover:bg-black/50"
                      >
                        <UIcon
                          name="i-lucide-loader-circle"
                          class="size-7 animate-spin"
                ...
                <div class="p-4">
                  <div class="flex items-start justify-between gap-3">
                    <h3
                      class="text-base font-bold leading-tight flex flex-wrap gap-2 items-center"
                    >
                      {{ project.title || "No title" }}
                      <span
                        v-if="project.type"
                        class="border border-yellow-500 bg-yellow-200 text-yellow-700 rounded-xl p-2 text-xs font-medium"
                        >{{ project.type }}</span
                      >
                    </h3>
                  </div>
                  ...
                  <!-- Footer -->
                  <div
                    class="mt-4 flex flex-col items-start justify-between gap-2 text-sm text-white/70"
                  >
        ...
        <div class="mt-8 flex items-center justify-between">
          <div class="text-sm text-white/70">
            Showing <span class="text-white">{{ startIndex + 1 }}</span
            >–<span class="text-white">{{ endIndex }}</span> of
            <span class="text-white">{{ filteredProjects.length }}</span>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <template v-for="(p, i) in pagesToShow" :key="`${p}-${i}`">
              ...
              <span
                v-else
                class="px-2 text-sm text-white/60 select-none"
                aria-hidden="true"
                >…</span
              >
            </template>
...
const endIndex = computed(() =>
  Math.min(totalCount.value, startIndex.value + pageSize.value)
);

/* Expose same names */
const paginated = computed(() => publicProjects.value);
const filteredProjects = computed(
  () => ({ length: totalCount.value } as unknown as PublicProject[])
);
